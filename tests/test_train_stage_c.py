...
def test_smoke_test_runs_end_to_end(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "scripts/train_stage_c.py", "--smoke-test",
         "--data", str(data_dir), "--weights", str(_WEIGHTS), "--img-size", "64"],
        capture_output=True, text=True, timeout=300,
    )
    assert result.returncode == 0, result.stdout + result.stderr
    assert "epoch 1/1" in result.stdout
    assert "loss=dice_bce" in result.stdout


def test_bce_weight_flag_is_respected(tmp_path):
    source_dir = tmp_path / "source"
    source_dir.mkdir()
    rng = np.random.default_rng(0)
    for i in range(4):
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert "bce_weight=0.75" in result.stdout


def test_writes_checkpoint_when_not_smoke_test(tmp_path):
    source_dir = tmp_path / "source"
    source_dir.mkdir()
    rng = np.random.default_rng(0)
    for i in range(4):
    ...
    result = subprocess.run(
        [sys.executable, "scripts/train_stage_c.py",
         "--data", str(data_dir), "--weights", str(_WEIGHTS), "--img-size", "64",
